...
def main():
    ...
    userName = sys.argv[2]
#junli.li - bugfix 23836
    lines = Utils.readFile(Globals.CIFS_VOLUME_FILE, lines=True)
    for line in lines:
        if not line.strip():
            continue
        if line.strip() == "admin users = %s" % (userName):
            print "Failed to remove the user:%s on gateway server. It has its own volume" % userName
	    Utils.log("Failed to remove the user:%s on gateway server. It has its own volume\n" % userName)
            sys.exit(2)
##
