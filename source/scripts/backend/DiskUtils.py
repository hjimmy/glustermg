...
def getHostName():
    rv = Utils.runCommand("hostname", output=True, root=True)
    if rv["Status"] == 0:
...
def getBrickInfo():
##  solve bug: cannot get brick info of /dev/dm-*
    mtabPartitionsDict = getMtabPartitions()
    neofsDict = {}
    neofsList = []
    for name, values in mtabPartitionsDict.iteritems():
	values["Server"] = getHostName()
	device = getDevice(name)
        if values["Brick Directory"] and values["Brick Directory"].startswith("/neofs/data"):
	    rv = Utils.runCommand(["df", values["Brick Directory"]], output=True)
	    values["Total Space(MB)"] = RoundFloat(long(rv["Stdout"].split("\n")[-2].split()[-5]) / 1024.0)
            values["Free Space(MB)"] = RoundFloat(long(rv["Stdout"].split("\n")[-2].split()[-3]) / 1024.0)
	    neofsDict[name] = mtabPartitionsDict[name]
    for name, values in neofsDict.iteritems():
	neofsList.append(values["Server"]+":"+values["Brick Directory"]+":"+values["Free Space(MB)"]+":"+values["Total Space(MB)"])
    return neofsList
        ...
        if values["MountPoint"]:
            ...
            if rv["Status"] == 0:
                try:
                    ...
                    pass
        if os.path.isdir("/sys/block/%s/device" % name):
...
def getMtabPartitions():
    mtabPartitionsDict = {}
    s = Utils.readFile("/etc/mtab", lines=True)
    for line in s:
        tokens = line.strip().split()
        mtabPartitionsDict[tokens[0]] = {"Brick Directory" : tokens[1]}
    return mtabPartitionsDict
