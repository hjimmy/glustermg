...
def getNetDeviceList(root=""):
    ...
    for deviceName in os.listdir("/sys/class/net/"):
        ...
        dname = deviceName
        if os.path.exists('/etc/sysconfig/network-scripts/ifcfg-' + deviceName):
            f = open('/etc/sysconfig/network-scripts/ifcfg-' + deviceName)
            reslt = f.read()
            lst = reslt.split('\n')
            tp = lst[3]
            tp=tp.split('=')
            if tp[-1] == 'OVSBridge':
                cmd = 'grep ' + deviceName + ' /etc/sysconfig/network-scripts/*|grep -v ifcfg-' + deviceName
                ss = os.popen(cmd).read()
                llst=ss.split(':')
                ss = llst[0].split('/')[-1].split('-')[-1]
                if ss.strip()!='':
                    dname = ss

        rv = Utils.runCommand("ethtool %s" % dname, output=True, root=True)
        if rv["Status"] == 0:
            for line in rv["Stdout"].split("\n"):
                if line.find("Speed: ") != -1:
                    netDevice["speed"] = line.split(": ")[1].upper().split("MB")[0]
